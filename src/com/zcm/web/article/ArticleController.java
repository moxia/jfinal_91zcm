	...
	public void search() throws Exception{
		...
		if(!"".equals(word)){
			...
			for(Article article : articleList.getList()){
				...
				long anum = Db.queryLong("select count(1) from gbook where aid = ? and state = 1",article.getInt("aid"));
				vo.setMsgnum(anum);
				category = Category.dao.findFirst("select * from category where cid = ?", article.getInt("cid"));
	...
	public void articleList(){
		...
		if(category==null){
			...
			if(top!=null){
				...
				long anum = Db.queryLong("select count(1) from gbook where aid = ? and state = 1",top.getInt("aid"));
				vo.setMsgnum(anum);
			...
			for(Article article:articleList.getList()){
				...
				vo.setCid(article.getInt("cid"));
				vo.setClicknum(article.getInt("clicknum"));
				long num = Db.queryLong("select count(1) from gbook where aid = ? and state = 1",article.getInt("aid"));
				vo.setMsgnum(num);
